...
# ===== NUEVO: usamos urllib en lugar de requests =====
...
def refreshToken():
    ...
    status, body = http_post(REFRESH_TOKEN_URL, data=data, headers=headers)

    try:
        response = json.loads(body.decode("utf-8"))
    except Exception:
        raise KeyError(f"Invalid response from Spotify token endpoint: {body!r}")
...
def get(url):
    ...
    # Token expirado, refrescamos y reintentamos
    if status == 401:
        SPOTIFY_TOKEN = refreshToken()
        headers = {"Authorization": f"Bearer {SPOTIFY_TOKEN}"}
        status, body = http_get(url, headers=headers)
    ...
    if status != 200:
        # Puedes ajustar este print según necesites
        raise Exception(f"{url} returned status {status}: {body.decode('utf-8', 'ignore')}")
...
def gradientGen(albumArtURL, color_count):
    status, body = http_get(albumArtURL)
    if status != 200:
        # Si falla, puedes usar el placeholder o un color por defecto
        status, body = http_get(PLACEHOLDER_URL)

    colortheif = ColorThief(BytesIO(body))
...
def loadImageB64(url):
    ...
    if status != 200:
        # fallback a placeholder de Unsplash si algo va mal
        status, body = http_get(PLACEHOLDER_URL)
